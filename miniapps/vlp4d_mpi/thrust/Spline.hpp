...
public:
  Spline(Config *conf) {
    ...
    fn_tmp_       = RealView4D("fn_tmp", shape_4d{nx_,ny_,nvx_,nvy_}, range_4d{nx_min_,ny_min_,nvx_min_,nvy_min_});
    fn_trans_.fill(0);
...
public:
  inline void computeCoeffCore_parallel_xy(RealView4D &fn, RealView4D &fn_tmp) {
    ...
    auto spline_2d = [=] MDSPAN_FORCE_INLINE_FUNCTION (const int i0, const int i1) {
    ...
    const int2 begin = make_int2(n0_min, n1_min);
    const int2 end   = make_int2(n0_max, n1_max);
    Impl::for_each<default_iterate_layout>(begin, end, spline_2d);
  }
  ...
  inline void computeCoeffCore_parallel_vxvy(RealView4D &fn, RealView4D &fn_tmp) {
    ...
    auto spline_2d = [=] MDSPAN_FORCE_INLINE_FUNCTION (const int i2, const int i3) {
      const float64 alpha = sqrt3 - 2;
      const float64 beta  = sqrt3 * (1 - alpha * alpha);
      #if defined( LONG_ENOUGH_BUFFER )
        float64 tmp1d[LONG_BUFFER_WIDTH];
    ...
    const int2 begin = make_int2(n2_min, n3_min);
    const int2 end   = make_int2(n2_max, n3_max);
    Impl::for_each<default_iterate_layout>(begin, end, spline_2d);
  }
};
