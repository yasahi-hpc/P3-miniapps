...
void Diags::compute(Config *conf, Efield *ef, int iter) {
  ...
  #if defined( ENABLE_OPENMP_OFFLOAD )
    #pragma omp target teams distribute parallel for simd collapse(2) map(tofrom:iter_mass,it_nrj,it_nrjx,it_nrjy) reduction(+:iter_mass,it_nrj,it_nrjx,it_nrjy)
  #else
...
void Diags::computeL2norm(Config *conf, RealView4D &fn, int iter) {
  ...
  float64 l2loc = 0.;
  #if defined( ENABLE_OPENMP_OFFLOAD )
    #pragma omp target teams distribute parallel for simd collapse(4) map(tofrom: l2loc) reduction(+:l2loc)
  #else
