...
void lag_basis(const double px, double * coef) {
  ...
  struct advect_1D_x_functor {
    Config* conf_;
    RealView4D fn_, fnp1_;
    double dt_;
    int s_nxmax, s_nymax, s_nvxmax, s_nvymax;
    double hmin, hwidth, dh, idh;
    double minPhix, minPhivx, dx, dvx;

    advect_1D_x_functor(Config*conf, const RealView4D fn, RealView4D fnp1, double dt)
  ...
  struct advect_1D_y_functor {
    ...
    advect_1D_y_functor(Config* conf, RealView4D fn, RealView4D fnp1, double dt)
      : conf_(conf), fn_(fn), fnp1_(fnp1), dt_(dt) {
  ...
  struct advect_1D_vx_functor {
    ...
    advect_1D_vx_functor(Config* conf, RealView4D fn, RealView4D fnp1, Efield* ef, double dt)
  ...
  struct advect_1D_vy_functor {
    ...
    advect_1D_vy_functor(Config* conf, RealView4D fn, RealView4D fnp1, Efield* ef, double dt)
      : conf_(conf), fn_(fn), fnp1_(fnp1), ef_(ef), dt_(dt) {
      const Domain *dom = &(conf_->dom_);
      s_nxmax  = dom->nxmax_[0];
      s_nymax  = dom->nxmax_[1];
  ...
  void advect_1D_x(Config *conf, const RealView4D &fn, RealView4D &fnp1, float64 dt) {
    ...
    int s_nvxmax = dom->nxmax_[2];
      ...
      MDPolicy<3> mdpolicy_3d( {{0,0,0}}, {{s_nxmax,s_nymax,s_nvxmax}}, {{TILE_SIZE0,TILE_SIZE1,TILE_SIZE2}} );
