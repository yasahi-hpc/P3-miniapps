...
namespace Advection {
  ...
  void advect_2D_xy(Config *conf, RealView4D &fn, RealView4D &fn_tmp, float64 dt) {
    ...
    auto advect_2d = [=](const int ix, const int iy, const int ivx, const int ivy) {
  ...
  void advect_4D(Config *conf, Efield *ef, RealView4D &fn, RealView4D &tmp_fn, float64 dt) {
    ...
    auto computeFeet = [=](float64 xstar[DIMENSION], const int pos[DIMENSION]) {
    ...
    auto interp_4d = [=](float64 xstar[DIMENSION]) {
      int ipos[DIMENSION];
      for(int j = 0; j < DIMENSION; j++) {
    ...
    auto advect_4d = [=](const int ix, const int iy, const int ivx, const int ivy) {
