...
private:
  void init(std::array<size_type, ND> extents,
    ...
    #if defined( ENABLE_OPENACC )
      #pragma acc enter data copyin(this)
      #pragma acc enter data create(data_[0:size_], strides_[0:rank_])
      #pragma acc update device(strides_[0:rank_])
    #endif
  }
  ...
  void free() {
    if( !is_empty_ ) {
      if( is_copied_) {
        #if defined( ENABLE_OPENACC )
          acc_detach( (void**) &data_ );
          acc_detach( (void**) &strides_ );
        #endif
      } else {
        #if defined( ENABLE_OPENACC )
          #pragma acc exit data delete(data_[0:size_], strides_[0:rank_]) // detach data
        #endif
