#ifndef __OPENMP_CUDA_FFT_HPP__
#define __OPENMP_CUDA_FFT_HPP__
...
namespace Impl {
  ...
  struct FFT {
    ...
    private:
    void init() {
      static_assert(std::is_same<array_layout, layout_contiguous_at_left>::value, "The input Layout must be LayoutLeft");
      nx1h_ = nx1_/2 + 1;
      nx2h_ = nx2_/2 + 1;
