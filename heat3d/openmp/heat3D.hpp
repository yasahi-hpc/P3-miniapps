...
void finalize(ScalarType time, 
  ...
  ScalarType sum = 0;
    ...
    #pragma omp target teams distribute parallel for simd collapse(3) map(tofrom:sum) reduction(+:sum)
  #else
  ...
  #if defined(ENABLE_OPENMP_OFFLOAD)
    std::cout << "OFFLOAD MODE" << std::endl;
  #endif
}
...
void step(const View3D<ScalarType> &u, View3D<ScalarType> &un) {
  ...
  const int nx = Config::nx, ny = Config::ny, nz = Config::nz;
    ...
    #if defined(ENABLE_OPENMP_OFFLOAD)
      #pragma omp target teams distribute parallel for simd is_device_ptr(_u, _un)
    #else
