...
void finalize(ScalarType time, 
  ...
  ScalarType L2_norm = sqrt( std::transform_reduce(std::execution::par_unseq,
                                                   un_ptr,
                                                   un_ptr + un.size(),
                                                   un_ptr, 
                                                   0.0 ) );
...
void step(const View3D<ScalarType> &u, View3D<ScalarType> &un) {
  ...
  const int nx = Config::nx, ny = Config::ny, nz = Config::nz;
    ...
    auto heat_eq = [=] (const int i, const int j, const int k) {
    ...
    auto heat_eq = [=] (const unsigned int idx) {
      const int ix = idx % nx;
