...
void to_csv(Config &conf, Comm &comm, View3DType &u, int iter, std::vector<Timer*> &timers) {
  if(iter % conf.freq_diag == 0) {
    ...
    RealView2DHost u_xy("u_xy", gnx, gny);
    RealView2DHost u_xyres("u_xyres", gnx, gny);
    ...
    if(comm.is_master()) {
      ...
      filename += u.label() + "_xy_it" + zfill(iter, 6) + ".csv";
