...
void set_inside_function(RealView2D shallow_copy_to_a_View) {
  ...
  #if defined(ENABLE_OPENACC)
    #pragma acc data present(shallow_copy_to_a_View)
    #pragma acc parallel loop independent collapse(2)
  #endif
  for(int j=0; j<m; j++) {
    for(int i=0; i<n; i++) {
...
TEST( VIEW, COPY_CONSTRUCTOR ) {
  ...
  #if defined(ENABLE_OPENACC)
    #pragma acc data present(reference)
    #pragma acc parallel loop independent collapse(2)
  #endif
  for(int j=0; j<m; j++) {
    for(int i=0; i<n; i++) {
...
TEST( VIEW, MOVE ) {
  ...
  #if defined(ENABLE_OPENACC)
    #pragma acc data present(moved_reference, reference)
    #pragma acc parallel loop independent collapse(2)
  #endif
  for(int j=0; j<m; j++) {
    for(int i=0; i<n; i++) {
...
TEST( VIEW, SWAP ) {
  ...
  #if defined(ENABLE_OPENACC)
    #pragma acc data present(a, b, a_ref, b_ref)
    #pragma acc parallel loop independent collapse(2)
  #endif
  for(int j=0; j<m; j++) {
    for(int i=0; i<n; i++) {
